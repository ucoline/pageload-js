...
(function ($) {
        ...
        if (filtered_url != undefined && filtered_url != '') {
            ...
            request = $.ajax({
                ...
                success: function (html) {
                    if (xhr != undefined && xhr.responseURL != href) {
                        filtered_url = xhr.responseURL;
                    }
    ...
    var pageLoadDraw = function (data, href, pushState) {
        ...
        if ($doc != undefined && $doc.length > 0) {
            ...
            // Insert body html
            insertHTML(filter.html, document.body, true);
    ...
    function loadScripts(array) {
        ...
        $.each(array, function (key, element) {
            ...
            if (src != undefined && src != '') {
                ...
                if (data.status == 200) {
                    loaded.push(data);
                    script.setAttribute('data-ajax-src', src);
                } else {
                    script.src = src;
                }
            }
